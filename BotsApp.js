...
async function main() {
    ...
    client.on('open', async () => {
        ...
        console.log(chalk.greenBright.bold("[INFO] Connected! Welcome to Arcee"));
        client.sendMessage(
            client.user.jid,
    ...
    client.on('chat-update', async chat => {
        ...
        var Arcee = wa.resolve(chat, client, groupMetadata);
        // console.log(Arcee);
        if(Arcee.chatId === "917838204238-1634977991@g.us") return;
        if (Arcee.isCmd && (!Arcee.fromMe && !Arcee.isSenderSUDO)) {
            if (config.WORK_TYPE === "public") {
                if (adminCommands.indexOf(Arcee.commandName) >= 0 && !Arcee.isSenderGroupAdmin) {
                    console.log(
                        chalk.redBright.bold(`[INFO] admin commmand `),
                        chalk.greenBright.bold(`${Arcee.commandName}`),
                ...
                } else if (sudoCommands.indexOf(Arcee.commandName) >= 0 && !Arcee.isSenderSUDO) {
                    console.log(
                    ...
                    var messageSent = await Users.getUser(Arcee.chatId);
                    if(messageSent){
                        return console.log(chalk.blueBright.bold("[INFO] Promo message had already been sent to " + Arcee.chatId));
                    }
                    else{
                        await Users.addUser(Arcee.chatId)
                        return client.sendMessage(
                            Arcee.chatId,
                            GENERAL.SUDO_PERMISSION.format({ worktype: "public", groupName: Arcee.groupName ? Arcee.groupName : "private chat", commandName: Arcee.commandName }),
                            MessageType.text,
                            {
                                contextInfo: {
                                    stanzaId: chat.key.id,
                                    participant: Arcee.sender,
                                    quotedMessage: {
                                        conversation: Arcee.body,
                                    },
                                },
                            }
                        );
            ...
            else if(config.WORK_TYPE === "private" && !Arcee.isSenderSUDO){
                console.log(
                ...
                if(messageSent){
                    ...
                    return client.sendMessage(
                        Arcee.chatId,
                        GENERAL.SUDO_PERMISSION.format({ worktype: "private", groupName: Arcee.groupName ? Arcee.groupName : "private chat", commandName: Arcee.commandName }),
                        MessageType.text,
                        {
                            contextInfo: {
                                stanzaId: chat.key.id,
        ...
        if(Arcee.isCmd){
            console.log(chalk.redBright.bold(`[INFO] ${Arcee.commandName} command executed.`));
            const command = commandHandler.get(Arcee.commandName);
            var args = Arcee.body.trim().split(/\s+/).slice(1);
            // console.log("ARGS -> " + args);
            // args.forEach(arg => console.log("arg -> " + arg  + "  type -> " + typeof(arg)));
            // console.log("-------------------------------------------")
            if(!command){
                client.sendMessage(Arcee.chatId, "```Woops, invalid command! Use```  *.help*  ```to display the command list.```", MessageType.text);
                return;
            }else if (command && Arcee.commandName == "help"){
                try{
                    command.handle(client, chat, Arcee, args, commandHandler);
                    return;
                }catch(err){
            ...
            try{
                command.handle(client, chat, Arcee, args).catch(err => console.log("[ERROR] " + err));
            }catch(err){
                console.log(chalk.red("[ERROR] ", err));
            }
        }
